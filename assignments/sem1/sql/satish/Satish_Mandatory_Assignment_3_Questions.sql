...
alter table website_stats add column day_dt date after day;
update website_stats set day_dt = str_to_date(day, '%d-%m-%Y');
alter table website_stats drop column day;
select day, str_to_date(day, '%d-%m-%y') from website_stats;
alter table website_stats rename column day_dt to day;
select * from web;
alter table web add column launch_dt date after launch_date;
update web set launch_dt = str_to_date(launch_date, '%d-%m-%Y');
alter table web drop column launch_date;
alter table web rename column launch_dt to launch_date;
*/
...
-----------------------------------------------------------------
/*
alter table play_store add column release_date date after released;
alter table play_store add column updated_date date;
update play_store set updated_date = str_to_date(updated, '%d-%m-%Y');
select str_to_date(updated, '%d-%m-%Y') from play_store;
alter table play_store drop column released;
alter table play_store drop column updated;
alter table play_store rename column release_date to released;
alter table play_store rename column updated_date to updated;
alter table sale add column dt date;
update sale set dt = str_to_date(date, '%d-%m-%Y');
alter table sale  drop column date;
alter table sale rename column dt to date;
...
# the sales consecutive number i.e. ranking of game as per the sale took place, so that the latest game sold gets number
# 1. Order the result by editor's rating of the game
...
-----------------------------------------------------------------
/* Importing csv files and table prepraration */
...
/* There is a typo in genre of Fight club - drame instead of drama */
...
/*
Following SQL considers amount collected on actual date
select rental_date, p.payment_date, payment_amounts from rent r left outer join
       (select payment_date, sum(payment_amount)over(partition by payment_date) payment_amounts
	   from rent) p on r.rental_date = p.payment_date
UNION
select rental_date, p.payment_date, payment_amounts from rent r right outer join
       (select payment_date, sum(payment_amount)over(partition by payment_date) payment_amounts
	   from rent) p on r.rental_date = p.payment_date;
*/
