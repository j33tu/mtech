...
/* This query was used to validate above result 
   select c.customer_id, customer_name, account_number, balance_amount from 
    BANK_CUSTOMER c inner join Bank_Account_Details a on c.customer_id = a.customer_id; */
...
/* Solution */
/* The following solution assumes calendar year. Hence months 1, 2, 3 as considered first quarter */
...
/* The following solution assumes financial year (April - March). Hence months 4, 5, 6 are considered as first quarter */
select a.customer_id, a.account_number, balance_amount, transaction_amount,
       transaction_amount, transaction_date from bank_account_details a inner join
       bank_account_transaction b on
       a.account_number = b.account_number
       where month(b.transaction_date) > 3  and month(b.transaction_date) < 7;
...
# select * from bank_account_relationship_details;
/* Solution */
# 3a Display records of All Accounts , their Account_types, the balance amount.
select customer_id, account_number, account_type, balance_amount from bank_account_details;
# 3b Along with first step, Display other columns with corresponding linking account number, account types
...
/* Solution */
#4a Display records of All Accounts , their Account_types, the balance amount.
select customer_id, account_number, account_type, balance_amount from bank_account_details;
#4b) Along with first step, Display other columns with corresponding linking account number, account types 
select a.customer_id, a.account_number, a.account_type, balance_amount, 
       linking_account_number, b.account_type account_type_b from bank_account_details a inner join
	   bank_account_relationship_details b on a.account_number = b.account_number;
# 4c) After retrieving all records of accounts and their linked accounts, display the  
#     transaction amount of accounts appeared  in another column.
select a.customer_id, a.account_number, a.account_type, balance_amount, 
...
select distinct account_number, 
				...
				(select sum(transaction_amount) from bank_account_transaction j where
                        ...
                        a;
# same solution using extract function
select distinct account_number, 
                (select sum(transaction_amount) from bank_account_transaction j where
						j.account_number = a.account_number and 
                        EXTRACT(YEAR_MONTH FROM transaction_date) = 202001 ) as Jan2020,
				(select sum(transaction_amount) from bank_account_transaction j where
						j.account_number = a.account_number and 
                        EXTRACT(YEAR_MONTH FROM transaction_date) = 202002 ) as Feb2020
...
#Q8. Display the first_name ,sex, age, fare and deck_number of the passengers
# having same deck number and sex as passenger number 7 but exclude the record
# of passenger number 7 from the display.(3 Rows)
...
/*
SQLs used after data import wizard to format date columns correctly
alter table yt3 rename youtube;
select * from youtube;

select convert(publish_date, date) from youtube;
select date(publish_date, '') from youtube;
select str_to_date(publish_date, '%d-%m-%Y'), str_to_date(trending_date, '%d-%m-%Y')  from youtube;

select str_to_date(publish_date, '%d-%m-%Y'), str_to_date(trending_date, '%d-%m-%Y')  from youtube;
alter table youtube add column publish_date_dt date after publish_date;
update youtube set publish_date_dt = str_to_date(trending_date, '%d-%m-%Y');
select * from youtube;
alter table youtube add column trending_date_dt date after trending_date;
update youtube set trending_date_dt = str_to_date(trending_date, '%d-%m-%Y');
select * from youtube;
create table youtube_backup as select * from youtube;
select * from youtube_backup;
alter table youtube drop column trending_date;
alter table youtube rename column trending_date_dt to trending_date;
alter table youtube drop column publish_date;
alter table youtube rename column publish_date_dt to publish_date;
select * from youtube;
*/
...
#select publish_date, trending_date 
...
#Q12. Write a query to display the channel_title, publish_date and the trending_date having category id
# in between 9 to Maximum category id .
